...
from toughpy import Attempt


def test_fixed_backoff():
    backoff = backoffs.FixedBackoff(3)
    assert backoff.get_delay(attempt=Attempt(1)) == 3
    assert backoff.get_delay(attempt=Attempt(10)) == 3


def test_fixed_list_backoff():
    backoff = backoffs.FixedListBackoff([2, 5, 8])
    assert backoff.get_delay(attempt=Attempt(1)) == 2
    assert backoff.get_delay(attempt=Attempt(2)) == 5
    assert backoff.get_delay(attempt=Attempt(3)) == 8

    for x in range(4, 100):
        assert backoff.get_delay(attempt=Attempt(x)) == 8
...
def test_random_backoff():
    ...
    for x in range(1, 100):
        delay = backoff.get_delay(attempt=Attempt(x))
...
def test_linear_backoff():
    ...
    assert backoff.get_delay(attempt=Attempt(1)) == 0.1
    assert backoff.get_delay(attempt=Attempt(2)) == 0.1 + 0.3
    assert backoff.get_delay(attempt=Attempt(3)) == 0.1 + 0.3 + 0.3

    for x in range(1, 100):
        assert backoff.get_delay(attempt=Attempt(x)) == 0.1 + 0.3 * (x - 1)
...
def test_exponential_backoff():
    ...
    assert backoff.get_delay(attempt=Attempt(1)) == 0.3
    assert backoff.get_delay(attempt=Attempt(2)) == 0.3 * 2 ** 1
    assert backoff.get_delay(attempt=Attempt(3)) == 0.3 * 2 ** 2

    for x in range(1, 10):
        assert backoff.get_delay(attempt=Attempt(x)) == 0.3 * 2 ** (x - 1)
...
def test_fibonacci_backoff():
    ...
    assert backoff.get_delay(attempt=Attempt(1)) == 1
    assert backoff.get_delay(attempt=Attempt(2)) == 2
    assert backoff.get_delay(attempt=Attempt(3)) == 3
    assert backoff.get_delay(attempt=Attempt(4)) == 5
    assert backoff.get_delay(attempt=Attempt(5)) == 8
    assert backoff.get_delay(attempt=Attempt(6)) == 13


def test_create_backoff():
    backoff = backoffs.create_backoff(given=None)
    assert isinstance(backoff, backoffs.FixedBackoff)
    assert backoff.get_delay(attempt=Attempt(10)) == 0.5

    backoff = backoffs.create_backoff(given=0.3)
    assert isinstance(backoff, backoffs.FixedBackoff)
    assert backoff.get_delay(attempt=Attempt(10)) == 0.3

    backoff = backoffs.create_backoff(given=[1, 2, 5])
    assert isinstance(backoff, backoffs.FixedListBackoff)
    assert backoff.get_delay(attempt=Attempt(10)) == 5
